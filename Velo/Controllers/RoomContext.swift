...
final class RoomContext {
    ...
    var shouldShowDisconnectReason: Bool = false
    public var latestError: LiveKitError?

    public let room = Room()

    var url: String = "wss://athena-22sisikp.livekit.cloud" {
    ...
    var dynacast: Bool = false {
        didSet { store.value.dynacast = dynacast }
    }

    var reportStats: Bool = false {
    ...
    var focusParticipant: Participant?

    var showMessagesView: Bool = false
    var messages: [ExampleRoomMessage] = []

    var textFieldString: String = ""

    var isVideoProcessingEnabled: Bool = false {
        didSet {
            if let _ = room.localParticipant.firstCameraVideoTrack as? LocalVideoTrack {
